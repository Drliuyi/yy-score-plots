...
The source tree contains no UKB participant data or EIDs. The locked Yin/Yang
fold manifests are deterministic runtime outputs generated from Huang's
`all.rds/prot.rds` when absent and validated by the normalized-content hashes
recorded in `score/config/fair.json`; they are not Git files.
